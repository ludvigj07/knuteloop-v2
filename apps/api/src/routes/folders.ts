...
const createFolderSchema = z.object({ name: z.string().trim().min(1).max(100) })
...
export const folderRoutes = new Hono<{ Variables: Variables }>()
  ...
  .post(
    ...
    async (c) => {
      ...
      const [created] = await tx.insert(knuteFolders).values({ schoolId, name }).returning()
  ...
  .patch(
    ...
    async (c) => {
      ...
      const patch: { name?: string; sortOrder?: number; updatedAt: Date } = { updatedAt: new Date() }
